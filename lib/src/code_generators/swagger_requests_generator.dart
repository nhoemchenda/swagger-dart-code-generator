...
class SwaggerRequestsGenerator extends SwaggerGeneratorBase {
    ...
    return Method(
      (m) => m
        ...
        ..optionalParameters.add(Parameter(
          (p) => p
            ..named = true
            ..type = Reference('Iterable<Interceptor>?')
            ..name = 'interceptors',
        ))
        ..optionalParameters.add(Parameter(
          (p) => p
  ...
  List<Method> _getAllMethodsContent({
    ...
    swaggerRoot.paths.forEach((String path, SwaggerPath swaggerPath) {
      swaggerPath.requests.forEach((String requestType, SwaggerRequest swaggerRequest) {
        ...
        if (options.includePaths.isNotEmpty && !options.includePaths.any((includePath) => RegExp(includePath).hasMatch(path))) {
          return;
        }
        ...
        final method = Method((m) => m
          ...
          ..name = methodName
          ..annotations.addAll(_getMethodAnnotation(requestType, path, hasOptionalBody, isMultipart))
  ...
  Code _generatePublicMethodCode(
    ...
    final parametersListString = parameters.map((p) {
      if (p.type!.symbol!.startsWith('enums.')) {
        ...
        final toStringPart = p.annotations.any((p0) => p0.code.toString().contains('symbol=Body')) ? '' : '?.toString()';
    ...
    allModels.toSet().forEach((model) {
      allModelsString += 'generatedMapping.putIfAbsent($model, () => $model.fromJsonFactory);\n';
    });
  ...
  String _createSummaryParameters(
    ...
    final description = [neededParameter.description, neededParameter.schema?.description].firstWhere((element) => element?.isNotEmpty == true, orElse: () => '')!.replaceAll(RegExp(r'\n|\r|\t'), ' ');
  ...
  List<Parameter> _getAllParameters({
    ...
    final securityParameters = swaggerRequest.security.map((e) => root.securityDefinitions[e]).whereNotNull();

    final additionalHeaders = options.additionalHeaders.map((e) => SwaggerRequestParameter(
          inParameter: 'header',
          name: e,
          type: 'String',
        ));
  ...
  bool _isEnumRef(String ref, SwaggerRoot root) {
    ...
    if ((neededSchema.type == kString || neededSchema.type == kInteger) && neededSchema.enumValues.isNotEmpty) {
      return true;
    }
  ...
  Code? _getHeaderDefaultValue(SwaggerRequestParameter swaggerParameter) {
    final overridenValue = options.defaultHeaderValuesMap.firstWhereOrNull((map) => map.headerName.toLowerCase() == swaggerParameter.name.toLowerCase());
  ...
  String? _getReturnTypeFromType(SwaggerResponse swaggerResponse, String modelPostfix) {
    ...
    if (responseType == kArray) {
      ...
      final arrayType = [itemsRef, itemsOriginalRef, itemsType, kObject].firstWhere((element) => element?.isNotEmpty == true)!;
  ...
  String? _getReturnTypeFromContent({
    ...
    if (schemaRef.isNotEmpty) {
      ...
      final typeName = getValidatedClassName(schemaRef.getRef()).withPostfix(modelPostfix);
    ...
    if (schemaItemsRef.isNotEmpty) {
      final result = getValidatedClassName(schemaItemsRef.getRef()).withPostfix(modelPostfix).asList();
    ...
    if (content.schema?.type == kArray) {
      ...
      if (itemsType == kArray && content.schema?.items?.items?.ref != null) {
        final itemsItemsType = content.schema?.items?.items?.ref.getRef() ?? content.schema?.items?.items?.type ?? kObject;
...
extension on SwaggerResponse {
  String get anyRef {
    final allRefs = [content?.ref, content?.schema?.ref, content?.schema?.items?.ref, schema?.ref, schema?.items?.ref, ref];
